...
<html lang="en">
  ...
  <div class="container">
    ...
    <a href="#" class="text-decoration-none text-light">Become a Seller</a>
      <a href="{{url('cart-list')}}" class="btn theme-green-btn btn-sm text-light ms-1 rounded-pill px-3 py-2"><i class="fa-solid fa-cart-shopping"></i> Cart</a>

       @if(Auth::check())
       <a href="{{url('home')}}" class="btn theme-orange-btn btn-sm text-light ms-1 rounded-pill px-3 py-2"><i class="fa-solid fa-user"></i>{{Auth::user()->email}}</a>
       <a href="{{url('logout')}}" class="btn theme-orange-btn btn-sm text-light ms-1 rounded-pill px-3 py-2"><i class="fa-solid fa-user"></i>Logout</a>
       @else
      ...
      <ul class="nav">
        ...
        @if(isset($categories))
            @foreach($categories as $category)
                <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle text-dark" href="{{ url('category/' . $category->name) }}" role="button" data-bs-toggle="dropdown">
                    {{ $category->name }}
                </a>
                @if($category->subcategories->count() > 0)
                    <ul class="dropdown-menu">
                    @foreach($category->subcategories as $subcategory)
                        <li><a class="dropdown-item" href="{{ url('category/' . $category->name . '/' . $subcategory->name) }}">{{ $subcategory->name }}</a></li>
                    @endforeach
                    </ul>
                @endif
                </li>
